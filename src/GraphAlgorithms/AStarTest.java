...
public class AStarTest
{
	...
	Hashtable<GraphNode<String>, Pair<GraphNode<String>, Integer>> table = AStar.aStar(
		graph, nA, nD, n -> {
		 Coord c = coord.get(n);
		 return (int)Math.hypot(c.getIntX()-300, c.getIntY()-100);
		});
	//DirectedGraph<String, Integer> path = Dijkstra.constructPathGraph(graph, table);
	...
	GNode<Coord> end = new GNode<>(new Coord(16, 11));
	
	Hashtable<GraphNode<Coord>, Pair<GraphNode<Coord>, Integer>> table = AStar.aStar(
		graph, start, end, n->{
		    Coord c = coord.get(n);
		    Coord d = coord.get(end);
		    int dist = (int)Math.hypot(c.getIntX()-d.getIntX(), c.getIntY()-d.getIntY());
		    return dist;
		});
